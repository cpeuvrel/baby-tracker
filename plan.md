...
- Minimal CI (`.github/workflows/ci.yml`: lint, tests, build for the app and for `functions/`). Dependabot (`.github/dependabot.yml`) opens weekly update PRs; minor/patch ones are auto-merged when CI passes (→ Vercel deploy), majors and failures @mention the repo owner (→ email). Cloud Functions are deployed by `.github/workflows/deploy-functions.yml` on every push to `main` touching `functions/` (service account key in the `FIREBASE_SERVICE_ACCOUNT` secret). Firestore rules are still deployed by hand. Vercel previews are added to Firebase Auth's authorized domains by `.github/workflows/preview-auth-domain.yml` (last 10 kept) so Google sign-in works on them — open a preview from the PR's "View deployment" button; note that previews use the real Firebase data.
